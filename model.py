...
class Employee(db.Model):
    ...
    dept_code = db.Column(db.String, db.ForeignKey("departments.dept_code"))

    dept = db.relationship("Department", back_populates="emps")
...
class Department(db.Model):
    ...
    emps = db.relationship("Employee", back_populates="dept")

    def __repr__(self):
        return f"<Dept id={self.dept_code} name={self.dept}>"
...
def all_employees_nav():
    """Returns list of all employees who have a department set.
    
    A list of Employee model instances will be returned.
    """
...
def all_employees_join():
    """For every employee, return the employee name, their department name, and the department phone number.

    Returns a list of tuples in the form (employee_name, department_name, department_phone)
    """
    ...
    # as in the function above, to load all the data needed.
    emps = db.session.query(Employee.name, Department.dept, Department.phone).join(
        Department
    )
...
# Query Answers:
# 1. Grab the employee object with the name ‘Cynthia Dueltgen’. 
#   cynthia = db.session.query(Employee).filter(Employee.name == 'Cynthia Dueltgen')

# 2. What department is she in? 
#   cynthia.dept

# 3. What is the phone number of the department that Cynthia is in?
#   cynthia.dept.phone

# 4. Find all employees whose department we don’t know.
...
#   db.session.query(Employee.name).filter(Employee.fav_color == 'orange').all()

# 6. Find all employees who work in ‘Administration’.
#   db.session.query(Department).filter(Department.dept == 'Administration').one().emps
